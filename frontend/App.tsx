...
// The URL for your backend API. Using an environment variable is best practice.
...
function App() {
  ...
  // Trigger fetch when formData changes
  useEffect(() => {
    // Debounce the request to avoid spamming the API on every keystroke
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <LanguageContext.Provider value={{ language, setLanguage, t, isRTL }}>
              ...
              <Grid item xs={12} md={8}>
                <Card>
                  <CardContent sx={{ minHeight: '500px' }}>
                    ...
                    {!isLoading && !error && !results && (
                      <Box display="flex" justifyContent="center" alignItems="center" height="100%">
                        <Typography variant="h6" color="text.secondary">{t('enterDataPrompt')}</Typography>
                      </Box>
                    )}
                  </CardContent>
                </Card>
        ...
        <Box component="footer" sx={{ py: 3, px: 2, mt: 'auto', backgroundColor: (theme) => theme.palette.mode === 'light' ? theme.palette.grey[200] : theme.palette.grey[800], textAlign: 'center' }}>
