 ...
 */
export function searchScenarios(scenarios: ScenarioData[], query: string): ScenarioData[] {
  ...
  return scenarios.filter((scenario) => scenario.name.toLowerCase().includes(trimmedQuery));
}
 ...
 */
export function filterScenarios(scenarios: ScenarioData[], filter: FilterOption): ScenarioData[] {
  if (filter === "all") {
    return scenarios;
  }
 ...
 */
export function sortScenarios(scenarios: ScenarioData[], sort: SortOption): ScenarioData[] {
  ...
  switch (sort) {
    ...
    case "outcome-best":
      return sorted.sort((a, b) => b.results.netOutcome - a.results.netOutcome);

    case "outcome-worst":
      return sorted.sort((a, b) => a.results.netOutcome - b.results.netOutcome);
