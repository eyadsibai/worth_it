...
interface ChartTooltipProps {
  ...
  payload?: Array<{
    value: number;
    payload: { name: string; valuation: number; confidence: number };
  }>;
}
    ...
    <Card className="border-0 shadow-[0_1px_2px_rgba(0,0,0,0.03),0_2px_8px_rgba(0,0,0,0.04)]">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-muted/50 grid grid-cols-3 gap-4 rounded-lg p-4">
          <div>
            <p className="text-muted-foreground text-xs tracking-wide uppercase">Minimum</p>
          ...
          <div>
            <p className="text-muted-foreground text-xs tracking-wide uppercase">Maximum</p>
            <p className="text-lg font-semibold tabular-nums">
              {maxVal.whole}
          ...
          <div>
            <p className="text-muted-foreground text-xs tracking-wide uppercase">Range</p>
            <p
              className={`text-lg font-semibold ${isWideRange ? "text-amber-500" : isTightRange ? "text-terminal" : ""}`}
            >
              ±{rangePctDisplay}%
            </p>
          </div>
        ...
        {comparison.insights.length > 0 && (
          <div className="space-y-2">
            <p className="text-muted-foreground text-xs tracking-wide uppercase">Insights</p>
            <div className="space-y-2">
...
function MethodCard({ result }: { result: FrontendValuationResult }) {
  ...
  return (
    <div className="bg-muted/30 rounded-lg p-4">
      ...
      {result.notes && (
        <p className="text-muted-foreground mt-2 line-clamp-2 text-xs">{result.notes}</p>
      )}
    </div>
  );
}
