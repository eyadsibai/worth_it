...
function App() {
  ...
  const fetchCalculations = useCallback(async (data: FormData) => {
    ...
    try {
      const response = await fetch(`${API_BASE_PATH}/calculate/`, {
        ...
        body: JSON.stringify({ ...data, rsu_params: data.rsu, options_params: data.options }),
      });
      if (!response.ok) {
  ...
  useEffect(() => {
    const handler = setTimeout(() => { fetchCalculations(formData); }, 500);
    return () => { clearTimeout(handler); };
  }, [formData, fetchCalculations]);

  const handleFormChange = (newFormData: FormData) => { setFormData(newFormData); };
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <LanguageContext.Provider value={{ language, setLanguage, t, isRTL }}>
        {/* The rest of your JSX and components go here */}
