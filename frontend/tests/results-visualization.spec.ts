...
test.describe("Results Visualization UI/UX", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector(
      '[data-testid="results-section"], .results-container, [class*="result"]',
      {
        timeout: 10000,
      }
    );
  ...
  test("should update results when form values change", async ({ page }) => {
    ...
    const initialCost = await page
      .locator('[data-testid*="total"], [class*="total"]')
      .first()
      .textContent();
  ...
  test("should have print-friendly results view", async ({ page }) => {
    ...
    if (await printButton.isVisible()) {
      ...
      await expect(
        page
          .locator('[data-testid="results-section"], .results-container, [class*="result"]')
          .first()
      ).toBeVisible();
  ...
  test("should display error states gracefully", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="results-section"], .results-container, [class*="result"]').first()
