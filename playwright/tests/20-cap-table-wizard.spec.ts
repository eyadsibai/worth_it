...
test.describe('Cap Table Wizard - Automatic Display', () => {
  ...
  test('should persist skip preference in localStorage', async ({ page }) => {
    ...
    // Reload page and verify wizard doesn't appear again
    await page.reload();
    await page.getByRole('tab', { name: /I'm a Founder/i }).click();

    // Wait for the main cap table UI (Add Stakeholder button)
    await expect(page.getByText(/Add Stakeholder/i)).toBeVisible({ timeout: 10000 });
...
test.describe('Cap Table Wizard - Complete Flow', () => {
  ...
  test('should complete full wizard flow with founders only', async ({ page }) => {
    ...
    await expect(page.getByText(/Do you have advisors/i)).toBeVisible({ timeout: 5000 });
    ...
    await expect(page.getByText(/Have you raised funding/i)).toBeVisible({ timeout: 5000 });
    ...
    await expect(page.getByText(/Add Stakeholder/i)).toBeVisible({ timeout: 5000 });
    await expect(page.getByText('Alice Founder')).toBeVisible();
    await expect(page.getByText('Bob Cofounder')).toBeVisible();
  });
  ...
  test('should complete wizard with SAFE funding', async ({ page }) => {
    ...
    // Fill in funding (SAFE is default type)
    await page.getByPlaceholder(/Investor name/i).first().fill('Acme Ventures');
...
test.describe('Cap Table Wizard - Manual Trigger', () => {
  test('should show wizard button after skipping', async ({ page }) => {
    await page.goto('/');
    await page.evaluate(() => localStorage.clear());
    await page.reload();
